...
const main = async () => {
  ...
  // const filesList = await getSeriesData('scp-series');
  // const file = await getSingleFile(filesTest[6]);
  // const files = await Promise.all(filesTest.map(async (file) => {
  //   return await getSingleFile(file);
  // }));

  // const allSeriesData = await Promise.all(seriesList.series.map(async (series) => {
  //   const filesList = await getSeriesData(series.slug);
  //   // const files = filesList.map(async (file) => {
  //   //   return await getSingleFile(file);
  //   // })
  //   const files = await Promise.all(filesList.map(async (file) => await getSingleFile(file)));
  //   return files
  // }));
