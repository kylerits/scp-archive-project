...
const getFile = async (slug) => {
  ...
  try {
    const pageInfo = await page.evaluate(() => {
      const pTags = [...document.querySelectorAll('#content-wrap #page-content > p')];
      let content = '';
      pTags.forEach(p => {
      ...
      return {
        ...
        image: [...document.querySelectorAll('#page-content .scp-image-block .image')][0].src,
        content: content,
      };
    });
...
const main = async () => {
  ...
  const filesTest = await getSeriesData('scp-series-4');
